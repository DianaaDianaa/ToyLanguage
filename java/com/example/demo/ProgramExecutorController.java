...
public class ProgramExecutorController {
    ...
    private void populateFileTableListView() {
        ProgramState programState = getCurrentProgramState();
        List<String> files = new ArrayList<>(Collections.singleton((Objects.requireNonNull(programState).getFileTable().getContent().keySet()).toString()));
    ...
    private void populateSymbolTableView() {
        ProgramState programState = getCurrentProgramState();
        MyIDict<String, Value> symbolTable = Objects.requireNonNull(programState).getSymTable();
        ...
        if (controller != null) {
            try {
                ...
                if (programStates.size() > 0) {
                    //controller.oneStepForAllPrg(controller.getProgramStates());
                    controller.oneStep();
                    populate();
